...
{object3D.additionalWalls && object3D.additionalWalls.length > 0 && object3D.additionalWalls.map((item, i)=>(<AdditionalWall map={item.mesh.map} colour={item.mesh.colour} width={item.geometry.width} height={item.geometry.height} rotation={item.mesh.rotation} position={item.mesh.position} key={i} scale={item.mesh.scale}/>))}
  </group>
</Stage>
 </Canvas>
